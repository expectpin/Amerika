...
export class AuthService {
  ...
  private processLoginResponse(response: LoginResponse, rememberMe?: boolean) {
    ...
    const decodedIdToken = jwtHelper.decodeToken(token) as IdToken;
    ...
    this.saveUserDetails(user, permissions, token, rememberMe);
  ...
  private   saveUserDetails(user: User, permissions: PermissionValues[], token: string, rememberMe: boolean) {
    if (rememberMe) {
      this.localStorage.savePermanentData(token, DBkeys.ACCESS_TOKEN);
      //this.localStorage.savePermanentData(refreshToken, DBkeys.REFRESH_TOKEN);
      //this.localStorage.savePermanentData(expiresIn, DBkeys.TOKEN_EXPIRES_IN);
      this.localStorage.savePermanentData(permissions, DBkeys.USER_PERMISSIONS);
      this.localStorage.savePermanentData(user, DBkeys.CURRENT_USER);
    } else {
      this.localStorage.saveSyncedSessionData(token, DBkeys.ACCESS_TOKEN);
